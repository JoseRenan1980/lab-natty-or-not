# Podcast — Natural ou Fake Natty? IA na prática

## 📒 Descrição
Criação de um podcast curto usando Inteligência Artificial Generativa, explorando o tema “Natural ou Fake Natty?” e provocando a audiência sobre autenticidade digital. O conteúdo foi produzido e editado para publicação como YouTube Shorts.

## 🎬 Arquivos do Projeto

- [🎧 Podcast em MP4](./assets/podcast.mp4)
- ![🖼️ Capa do Podcast](./assets/capa.png)


## 🤖 Tecnologias utilizadas
- IA generativa para roteiro
- Clipchamp para edição
- Música livre de direitos autorais
- YouTube Shorts para publicação

## 🧐 Processo de criação
1. Ideia baseada no desafio da DIO.
2. Roteiro criado com ajuda de IA.
3. Produção do áudio com capa estática.
4. Troca da música de fundo para evitar bloqueio.
5. Publicação no YouTube.

## 🚀 Resultados
- Vídeo publicado: https://youtu.be/vhBVlDuRS00?si=r6sdnh9V2IR5TGpU
- Formato: Shorts (<60s)
- Hashtag: #LabDIONattyOrNot

## 💭 Reflexão
Criar algo “natty” com IA é sobre transparência: usar a tecnologia como ferramenta, sem mascarar a autoria. O maior aprendizado foi lidar com direitos autorais — trocar a trilha por música realmente livre garantiu publicação sem bloqueios.
